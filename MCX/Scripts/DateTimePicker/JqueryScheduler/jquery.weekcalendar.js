...
(function ($) {
    ...
    $.widget("ui.weekCalendar", {
        ...
        changeBranch: function () {//alert('Called Change Branch from Jquery.WeekCalender.js')
            ...
            this.options.usersData = this.options.GetUsers(BranchDD.val());
        ...
        changeDateNav: function () {
            //alert('Called From changeDateNav function in Jquery.WeekCalender.js');
        ...
        _renderCalendar: function () {
            ...
            if (options.buttons) {
                //calendarNavHtml = "<div class=\"wc-nav\"><select id='_branchDD'><option value='-1' selected = 'selected'>-Select Branchhhhhh-</option>";
                calendarNavHtml = "<div class=\"wc-nav\"><select id='_branchDD'></option>";//Added this line and commented above by Pradeep on 20 Jan2014
            ...
            calendarHeaderHtml = "<table class=\"wc-header\"><tbody><tr><td class=\"wc-time-column-header\">Name\\Date</td>";
            ...
            debugger;
            var start = 0;
            //----Start Added on 20 Jan 2014 by Pradeep
            var BranchDD = $("#_branchDD");
            this.options.usersData = this.options.GetUsers(BranchDD.val());
           
            //----End Added on 20 Jan 2014 by Pradeep
            var end = options.usersData.length;

            for (var i = start; i < end; i++) {
                calendarBodyHtml += "<tr><td class=\"wc-grid-timeslot-header\" UID=" + options.usersData[i].UserId + ">";
                calendarBodyHtml += options.usersData[i].UserName + "</td>";
        ...
        _setupEventCreationForWeekDay: function ($weekDay) {
            ...
            $weekDay.mousedown(function (event) {
                ...
                if ($newEvent.length) {
                    ...
                    var userId = $target.closest("tr").find(".wc-grid-timeslot-header").attr("uid");
                    var attendanceValue = "";


                    var newCalEvent = { start: eventDuration.start, end: eventDuration.end, title: options.newEventText, userId: userId, attendanceValue: attendanceValue };
        ...
        _renderEvents: function (events, $weekDayColumns) {
            ...
            $.each(eventsToRender, function (i, calEvent) {
                ...
                var userHeader = "";
                self.element.find(".wc-time-slots td.wc-grid-timeslot-header").each(function () {
                    if ($(this).attr("uid") == calEvent.userId) {
        ...
        _refreshEventDetails: function (calEvent, $calEvent) {
            ...
            var _User = jQuery.grep(options.usersData, function (x) {
                return x.UserId == calEvent.userId;
            });

            var Branch = jQuery.grep(options.branches, function (x) {
                return x.BranchId == _User[0].BranchId;
            });

            var _title = [];
            _title = jQuery.grep(options.attendanceType, function (x) {
                return x.AttendanceTypeId == calEvent.attendanceValue;
            });
